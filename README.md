...
> **認証について**: 現状Web画面・PDF配信ともに社内メールドメイン制限などの認証は未実装です。今回は本番の会社ドメイン（Google Workspace等）が用意されていないため実装を見送りました。検討した実装案は [`docs/architecture.md`](./docs/architecture.md#4-認証境界の現状重要) を参照してください。
